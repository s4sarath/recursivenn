...
class RNN:
    ...
    def forwardProp(self,node,correct=[], guess=[]):
        ...
        if node.isLeaf:
            ...
            l_cost, l_total = self.forwardProp(node.left, correct, guess)
            r_cost, r_total = self.forwardProp(node.right, correct, guess)
            
            total = l_total + r_total
            cost = l_cost + r_cost
            
            z = self.W.dot(np.hstack([node.left.hActs1, node.right.hActs1])) + self.b
            node.hActs1 = z * (z > 0)
        
        node.probs = self.Ws.dot(node.hActs1) + self.bs
